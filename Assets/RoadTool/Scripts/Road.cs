...
/*
 * Creates and stores road assets:
 * 	- mesh
 * 	- NEW (enabled with #define ROAD_LINES): RoadLines, which describe the left/right/center lines of the road.
 */
#define ROAD_LINES
...
public class Road : MonoBehaviour 
{
...
#if ROAD_LINES
	public bool generate_roadlines = false; 
#endif
	...
	public void Refresh()
	{
		...
		if (generate_roadlines){
			// create new roadlines instance
			RoadLines asset = ScriptableObject.CreateInstance<RoadLines> ();		
			...
			{
				int l = 4*i;
				// left and right re swapped here (and below)
				// not sure why, but it works
				asset.left.Add (v[l+0]);
				asset.right.Add (v[l+1]);
				if (i+1 == segs){ // last one, add to end
					asset.left.Add (v[l+2]);
					asset.right.Add (v[l+3]);
				}
			}
			//center is mean of left and right
			for (int i = 0; i<asset.left.Count; i++)
				asset.center.Add((asset.left[i]+asset.right[i])/2f);
			
			// save asset	
			//string folderPath = "Assets/RoadLines";
			...
			string assetPathAndName = "Assets/RoadLines/" + typeof(RoadLines).ToString() + " " + this.gameObject.GetInstanceID().ToString() + ".asset";

			//if (!AssetDatabase.IsValidFolder("Assets/RoadLines"))
			//	AssetDatabase.CreateFolder("Assets","RoadLines");
